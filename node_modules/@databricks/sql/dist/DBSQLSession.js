...
function getQueryParameters(sessionHandle, namedParameters, ordinalParameters) {
    ...
    if (!sessionHandle.serverProtocolVersion ||
        sessionHandle.serverProtocolVersion < TCLIService_types_1.TProtocolVersion.SPARK_CLI_SERVICE_PROTOCOL_V8) {
        throw new thrift_1.Thrift.TProtocolException(thrift_1.Thrift.TProtocolExceptionType.BAD_VERSION, 'Parameterized operations are not supported by this server. Support will begin with server version DBR 14.1');
    }
    const result = [];
...
class DBSQLSession {
    ...
    executeStatement(statement, options = {}) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            yield this.failIfClosed();
            const driver = yield this.context.getDriver();
            const operationPromise = driver.executeStatement(Object.assign(Object.assign(Object.assign({ sessionHandle: this.sessionHandle, statement, queryTimeout: options.queryTimeout, runAsync: true }, getDirectResultsOptions(options.maxRows)), getArrowOptions()), { canDownloadResult: (_a = options.useCloudFetch) !== null && _a !== void 0 ? _a : globalConfig_1.default.useCloudFetch, parameters: getQueryParameters(this.sessionHandle, options.namedParameters, options.ordinalParameters) }));
            const response = yield this.handleResponse(operationPromise);
